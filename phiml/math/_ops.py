...
def concat_tensor(values: Union[tuple, list], dim: str) -> Tensor:
    ...
    if any([v._is_tracer for v in values]):
        tracer = [v for v in values if v._is_tracer][0]
        return tracer.__concat__(values, dim)
...
def pad_to_uniform(x: Tensor, target_shape: Shape = EMPTY_SHAPE, pad_value=0):
    """
    ...
    sizes = [target_shape.get_size(d.name) if d.name in target_shape else (d.size if isinstance(d.size, int) else d.size.max) for d in inner_shape]
    natives = [t.native(inner_shape.names) for t in x._tensors]
    native_result = x.backend.pad_stack(natives, sizes, pad_value)
...
def maximum(x: Union[Tensor, float], y: Union[Tensor, float], allow_none=False):
    ...
    if isinstance(x, Shape) or isinstance(y, Shape):
        if isinstance(y, Shape) and not isinstance(x, Shape):
            x, y = y, x
        if isinstance(y, Number):
            return x.with_sizes([max(s, y) for s in x.sizes])
...
def minimum(x: Union[Tensor, float], y: Union[Tensor, float], allow_none=False):
    ...
    if isinstance(x, Shape) or isinstance(y, Shape):
        if isinstance(y, Shape) and not isinstance(x, Shape):
            x, y = y, x
        if isinstance(y, Number):
            return x.with_sizes([min(s, y) for s in x.sizes])
        elif isinstance(y, Tensor):
            x = y._tensor(x)
    if allow_none:
        if x is None:
...
def scatter(base_grid: Union[Tensor, Shape],
    ...
    * Jax: [`jax.lax.scatter_add`](https://jax.readthedocs.io/en/latest/_autosummary/jax.lax.scatter_add.html), [`jax.lax.scatter`](https://jax.readthedocs.io/en/latest/_autosummary/jax.lax.scatter.html)
    ...
    if index_dim and index_dim.labels[0]:
        ...
        assert not index_dim or index_dim.size == indexed_dims.rank
    if not index_dim:
