...
class CompressedSparseMatrix(Tensor):
    ...
    def _native_csr_components(self, invalid='clamp', get_values=True):
        ...
        channels = non_instance(self._values).without(ind_batch)
        native_indices = self._indices._reshaped_native([ind_batch, instance(self._indices)])  # allow variable instance size (PyTorch tracing)
        native_pointers = self._pointers._reshaped_native([ind_batch, instance(self._pointers)])
        native_values = self._values._reshaped_native([ind_batch, instance(self._values), channels]) if get_values else None
