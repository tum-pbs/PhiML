"""
Magic methods allow custom classes to be compatible with various functions defined in `unifyml.math`, analogous to how implementing `__hash__` allows objects to be used with `hash()`.
The magic methods are grouped into purely declarative classes (interfaces) by what functionality they provide.

* `Shaped` objects have a `unifyml.math.Shape`.
...
class Shaped(metaclass=_ShapedType):
    ...
    In either case, the returned shape must be an instance of `unifyml.math.Shape`.

    To check whether an object is `Shaped`, use `isinstance(obj, Shaped)`.

    **Usage in `unifyml.math`:**

    The functions `unifyml.math.shape` as well as dimension filters, such as `unifyml.math.spatial` or `unifyml.math.non_batch` can be called on all shaped objects.
...
class Sliceable(metaclass=_SliceableType):
    ...
    **Usage in `unifyml.math`:**
    ...
    In addition to slicing, sliceable objects can be unstacked along one or multiple dimensions using `unifyml.math.unstack`.
...
class Shapable(metaclass=_ShapableType):
    ...
    * `unifyml.math.concat`
    * `unifyml.math.expand`
    * `unifyml.math.rename_dims`
    * `unifyml.math.pack_dims`
    * `unifyml.math.unpack_dim`
    * `unifyml.math.flatten`

    Additionally, the `unifyml.math.BoundDim` syntax for dimension renaming and retyping is enabled, e.g. `obj.dim.as_channel('vector')`.
    """
    @staticmethod
...
class PhiTreeNode(metaclass=_PhiTreeNodeType):
    ...
    `unifyml.math.Tensor` instances as well as PyTree nodes (`tuple`, `list`, `dict` with `str` keys) are Φ-tree nodes.
    ...
    **Usage in `unifyml.math`:**
    ...
    Disassembly and assembly of Φ-tree nodes uses `unifyml.math.copy_with` which will call `__with_attrs__` if implemented.
    """
    ...
    def __with_attrs__(self, **attrs):
        """
        Used by `unifyml.math.copy_with`.
...
class BoundDim:
    ...
    ```python
    from unifyml.math.magic import BoundDim
    ...
    * `for slice in obj.dim1.dim2...` loops over all slices as if unstacking first
    ...
    def __init__(self, obj, name: str):
        ...
        assert isinstance(obj, Sliceable) and isinstance(obj, Shaped), f"Cannot create BoundDim for {type(obj).__name__}. Objects must be Sliceable and Shaped, see https://holl-.github.io/UnifyML/unifyml/math/magic.html"
        self.obj = obj
        self.name = name
...
def slicing_dict(obj, item) -> dict:
    ...
    if isinstance(item, tuple):
        if item[0] == Ellipsis:
            ...
            if len(item) > 1:
                warnings.warn("NumPy-style slicing for more than one channel dimension is highly discouraged. Use a dict or the special slicing syntax value.dim[slice] instead. See https://holl-.github.io/UnifyML/Math.html", SyntaxWarning, stacklevel=3)
            ...
            raise AssertionError(f"Cannot slice {obj}[{item}]. Use a dict or the special slicing syntax value.dim[slice] instead. See https://holl-.github.io/UnifyML/Math.html")
    else:
