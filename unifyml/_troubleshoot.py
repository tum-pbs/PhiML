...
def assert_minimal_config():  # raises AssertionError
    import sys
    assert sys.version_info.major == 3 and sys.version_info.minor >= 6, "UnifyML requires Python 3.6 or newer to run"

    try:
        import numpy
    except ImportError:
        raise AssertionError("UnifyML is unable to run because NumPy is not installed.")
    try:
        import scipy
    except ImportError:
        raise AssertionError("UnifyML is unable to run because SciPy is not installed.")
    from . import math
    with math.NUMPY:
        a = math.ones()
...
def troubleshoot():
    ...
    return f"UnifyML {__version__} at {dirname(__file__)}\n"\
    ...
    if gpu_count == 0:
        ...
        if librariesLoaded:
            ...
            if platform.system().lower() != 'linux':
                ...
                cuda_str = f"Optional TensorFlow CUDA kernels not available. GPU will be used nevertheless. Clone the UnifyML source from GitHub and run 'python setup.py tf_cuda' to compile them. See https://holl-.github.io/UnifyML/Installation_Instructions.html"
        return f"Installed ({tf_version}), {gpu_count} GPUs available.\n{cuda_str}"
